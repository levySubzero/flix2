...
const EpisodeCard: React.FC<EpisodeCardProps> = ({ data }) => {
  ...
  const addToSeries = useCallback(() => router.push(`/seriess/${data.id}`), [router, data.id]);

  return (
    <div className="group bg-zinc-900 col-span relative h-[12vw]">
      <img onClick={addToSeries} src={data.thumbnailUrl} alt="Series" draggable={false} className="
        cursor-pointer
        object-cover
        transition
        duration
      ...
      ">
        ...
        <div className="
          ...
          <div className="flex flex-row items-center gap-3">
            {/* <div onClick={addToSeries} className="cursor-pointer w-6 h-6 lg:w-10 lg:h-10 bg-white rounded-full flex justify-center items-center transition hover:bg-neutral-300">
